...
*/			    /* Booking from the closed month / qtr quota */
/*			   case 
			   		when (SR.Quota is null) and (SR.Achievement is null) then 'No M1 plan'
					when (SR.Quota = 0) then '$0 M1 Quota'   --if someone has $0 quota, any achievement is 100%
					else 'M1 QBH'
		...
		UNION
*/
		/* FY17 till FY21 Annual Attainment */
		select SR.EmployeeID, SR.FiscalYear, SR.Period, SR.Quota [M1 Quota], SR.Achievement [M1 Achievement], SR.Attainment [M1 Attainment],
			   case
		...
		/* FY19 to FY21 Quarterly Attainment */
...
#FY_M1_Attainment_2021 as (
	select Name, EmployeeID, Attainment [2021 M1 Attainment], Achievement [2021 M1 Achievement], Quota [2021 M1 Quota]
	from SalesOps_DM.dbo.StackRank_History
	where Period = 'FY' and Measure = 'M1' and FiscalYear in ('2021') 
),

#YtD_M1_Attainment_2022 as (
...
*/
	...
	from SalesOps_DM.dbo.StackRank SR
	where Period = 'CQ' and Measure = 'M1' and FiscalYear in ('2022')
),
...
#M1_Attainment_Wide as (
		select Org.*,
			 ...
			 case when [2017 M1 Attainment] = 0 then null
			      when [2017 M1 Attainment] >= 1 then 1 else 0 end [2017_Pass_M1_Attainment],
			 ...
			 case when [2018 M1 Attainment] = 0 then null
			 	  when [2018 M1 Attainment] >= 1 then 1 else 0 end [2018_Pass_M1_Attainment],
			 ...
			 end [2020_Pass_M1_Attainment],
			 
			 [2021 M1 Achievement], [2021 M1 Quota],
			 case when [2021 M1 Quota] = 0 then null else [2021 M1 Attainment] end [2021 M1 Attainment],
			 case when [2021 M1 Quota] = 0 then null else 
			 	  case when [2021 M1 Attainment] >= 1 then 1 else 0 end
			 end [2021_Pass_M1_Attainment]
...
--			 [2021 YtD Attainment],  [2021 YtD Achievement], [2021 FY Quota]
		...
		left join #FY_M1_Attainment_2021 #21 on #21.EmployeeID = Org.EmployeeID
--		left join #YtD_M1_Attainment_2022 #22 on #22.EmployeeID = Org.EmployeeID
)
...
--Select * from #M1_Achievement
--where EmployeeID = '100312'
