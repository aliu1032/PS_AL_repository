...
#StackRank_FY as (
	select Report_Date, Name, EmployeeID, 'FY' + right(FiscalYear,2) [Period], Measure, CQ_YtD_Achievement [SR_FY_M1_Achievement], CQ_YtD_Quota [SR_FY_M1_Quota]
	from SalesOps_DM.dbo.StackRank
	where Measure = 'M1' and Period in ('CQ')
),

#StackRank_FQ as (
	select Report_Date, Name, EmployeeID, 'FY' + Right(FiscalYear,2) + ' ' + Period [Period], Measure, Achievement [SR_Qtr_M1_Achievement], Quota [SR_Qtr_M1_Quota]
	from SalesOps_DM.dbo.StackRank
	where Measure = 'M1' and Period in ('Q1','Q2','Q3','Q4')
)	  
...
Select A.*
	...
	, Case
	  ...
	  , #StackRank_FY.[SR_FY_M1_Achievement]
	  , #StackRank_FY.[SR_FY_M1_Quota]
	  , #StackRank_FQ.[SR_Qtr_M1_Achievement]
	  , #StackRank_FQ.[SR_Qtr_M1_Quota]
		...
		from (	  
			...
			--	, [Final].[District_Permission]
				...
				, [Final].Stage, [Final].POC_Stage, [Final].POC_Status, [Final].StageGroup
			...
			--		, District_Permission = coalesce (left(Acct_Exec_Territory_ID, 18), left(SE_Quota.SE_District_ID, 18)) 
					...
					, Oppt.Partner_SE_Engagement_Level__c [Partner SE Engagement Level]
			...
			*/
					...
					, [Oppt Close Year] = 'FY' + substring(CloseDate_445.FiscalYear,3,2)
					, [Oppt Close Quarter] = 'FY'+ substring(CloseDate_445.FiscalYear, 3,2) + ' ' + CloseDate_445.FiscalQuarterName
		...
		left join #StackRank_FY on #StackRank_FY.EmployeeID = A.SE_Oppt_Owner_EmployeeID and #StackRank_FY.[Period] = A.[Close Year]
		left join #StackRank_FQ on #StackRank_FQ.EmployeeID = A.SE_Oppt_Owner_EmployeeID and #StackRank_FQ.[Period] = A.[Close Quarter]


where
--[Final].SE_Oppt_Owner = 'Andrew Stelow'
--and [Final].[Close Quarter] = 'FY21 Q3'
A.SE_Oppt_Owner = 'Reid Moncrief'
-- [Final].Id = '0060z000022phGWAAY'
--[Final].Acct_Exec in ('Chris Dopp')S
--[Final].SE_Oppt_Owner_EmployeeID= '101912'
