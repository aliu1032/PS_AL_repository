...
#SE_Org as (
/*		select [Employee ID], [Preferred Name], [Manager ID], [Manager Name], R.Resource_Group, [Title]
		from GPO_TSF_Dev.dbo.vemporgmapper O
		left join SalesOps_DM.dbo.SE_User_Role R on R.EmployeeID = O.[Employee ID]
*/
		SELECT cast(EmployeeID as varchar) [Employee ID], PreferredName [Preferred Name], [Manager ID], [Manager] [Manager Name], [Role] Resource_Group, [Title]
...
/* SE Quota */
#SE_M1_Quota as (
	select Q.Name, Q.EmployeeID, Q.Resource_Group, Q.[Year], Q.[Period], cast(Q.Quota as decimal(18,2)) Quota, (D.Territory_ID + '_001') SE_District_ID /* Id to pull the Geo Quota */
	from SalesOps_DM.dbo.SE_Org_Quota Q
	left join (Select Territory_ID, District from SalesOps_DM.dbo.TerritoryID_Master where Level = 'District') D on D.District = Q.District
	where Measure = 'M1'
...
#SE_Quota as (
	Select Q.Name SE_Name, Q.EmployeeID, Q.Resource_Group, Q.SE_District_ID, Q.[Year] [Year], Q.Period, Q.Half_Period,
	...
	from ( /* select SE users carry a Quota */ 
		...
		where Period in ('Q1','Q2','Q3','Q4') 
			  and EmployeeID in (Select distinct([Employee ID]) from #SE_Org)
		) Q
...
--	left join #Geo_Quota_Wide Geo on Geo.Territory_ID = Q.SE_District_ID and Geo.Period = Q.Period
),
...
#SE_Anaplan_Quota as (
	select Q.*
			...
			, Geo.Super_Region, Geo.SuperRegion_Qtrly_Quota, Geo.SuperRegion_Half_Quota, Geo.SuperRegion_Annual_Quota
			, Geo.SuperRegion_Qtrly_FB_Quota, Geo.SuperRegion_Half_FB_Quota, Geo.SuperRegion_Annual_FB_Quota
	...
	from (
			select [Employee ID] [EmployeeID], [Workday Employees E1] [SE_Name], [Manager],
				   ...
				   'Q1' Period, '1H' Half_Period, 'FY21' [Year]
			...
			select [Employee ID] [EmployeeID], [Workday Employees E1] [SE_Name], [Manager],
				   ...
				   [Measure 2 Q1 Assigned Quota] + [Measure 2 Q2 Assigned Quota] [SE_Half_FB_Quota],
				   [Measure 2 Q1 Assigned Quota] + [Measure 2 Q2 Assigned Quota] + [Measure 2 Q3 Assigned Quota] + [Measure 2 Q4 Assigned Quota] as [SE_Annual_FB_Quota],
				   'Q2' Period, '1H' Half_Period, 'FY21' [Year]
			from Anaplan_DM.dbo.Employee_Territory_And_Quota
			...
			select [Employee ID] [EmployeeID], [Workday Employees E1] [SE_Name], [Manager],
				   ...
				   [Measure 2 Q1 Assigned Quota] + [Measure 2 Q2 Assigned Quota] + [Measure 2 Q3 Assigned Quota] + [Measure 2 Q4 Assigned Quota] as [SE_Annual_FB_Quota],
				   'Q3' Period, '2H' Half_Period, 'FY21' [Year]
			from Anaplan_DM.dbo.Employee_Territory_And_Quota
			...
			select [Employee ID] [EmployeeID], [Workday Employees E1] [SE_Name], [Manager],
				   ...
				   [Measure 2 Q3 Assigned Quota] + [Measure 2 Q4 Assigned Quota] [SE_Half_FB_Quota],
				   [Measure 2 Q1 Assigned Quota] + [Measure 2 Q2 Assigned Quota] + [Measure 2 Q3 Assigned Quota] + [Measure 2 Q4 Assigned Quota] as [SE_Annual_FB_Quota],
				   'Q4' Period, '2H' Half_Period, 'FY21' [Year]
			from Anaplan_DM.dbo.Employee_Territory_And_Quota
		) Q 
...
/*
select *
from #SE_Anaplan_Quota
where EmployeeID = '104457'
--SE_Name = 'Reid Moncrief'
--where EmployeeID = '102944' --Dean Brady
--EmployeeID = '105598' --(Demetri Diakakis)
--EmployeeID = '103153' --(Bogusz Blaszkiewicz)
-- 'Philip Kalaf'Philip '104942' ---this is not a SE
-- Chris Otis '104457'
*/
...
--		, CreateDate_445.FiscalQuarterName [Fiscal_CreateQuarter]
		...
		, Super_Region = coalesce(AE_Quota.Super_Region, SE_Quota.Super_Region)
		, SuperRegion_Qtrly_Quota = coalesce(AE_Quota.SuperRegion_Qtrly_Quota, SE_Quota.SuperRegion_Qtrly_Quota)
		, SuperRegion_Half_Quota = coalesce(AE_Quota.SuperRegion_Half_Quota, SE_Quota.SuperRegion_Half_Quota)
		, SuperRegion_Annual_Quota = coalesce(AE_Quota.SuperRegion_Annual_Quota, SE_Quota.SuperRegion_Annual_Quota)

		, SuperRegion_Qtrly_FB_Quota = coalesce(AE_Quota.SuperRegion_Qtrly_FB_Quota, SE_Quota.SuperRegion_Qtrly_FB_Quota)
		, SuperRegion_Half_FB_Quota = coalesce(AE_Quota.SuperRegion_Half_FB_Quota, SE_Quota.SuperRegion_Half_FB_Quota)
		, SuperRegion_Annual_FB_Quota = coalesce(AE_Quota.SuperRegion_Annual_FB_Quota, SE_Quota.SuperRegion_Annual_FB_Quota)
...
/*		, District = Geo_Quota.District
		, District_Qtrly_Quota = Geo_Quota.District_Qtrly_Quota
		, District_Half_Quota = Geo_Quota.District_Half_Quota
		, District_Annual_Quota = Geo_Quota.District_Annual_Quota

		, District_Qtrly_FB_Quota = Geo_Quota.District_Qtrly_FB_Quota
		, District_Half_FB_Quota = Geo_Quota.District_Half_FB_Quota
		, District_Annual_FB_Quota = Geo_Quota.District_Annual_FB_Quota
	
		, Region = Geo_Quota.Region
		, Region_Qtrly_Quota = Geo_Quota.Region_Qtrly_Quota
		, Region_Half_Quota = Geo_Quota.Region_Half_Quota
		, Region_Annual_Quota = Geo_Quota.Region_Annual_Quota

		, Region_Qtrly_FB_Quota = Geo_Quota.Region_Qtrly_FB_Quota
		, Region_Half_FB_Quota = Geo_Quota.Region_Half_FB_Quota
		, Region_Annual_FB_Quota = Geo_Quota.Region_Annual_FB_Quota
	
		, Super_Region = Geo_Quota.Super_Region
		, SuperRegion_Qtrly_Quota = Geo_Quota.SuperRegion_Qtrly_Quota
		, SuperRegion_Half_Quota = Geo_Quota.SuperRegion_Half_Quota
		, SuperRegion_Annual_Quota = Geo_Quota.SuperRegion_Annual_Quota

		, SuperRegion_Qtrly_FB_Quota = Geo_Quota.SuperRegion_Qtrly_FB_Quota
		, SuperRegion_Half_FB_Quota = Geo_Quota.SuperRegion_Half_FB_Quota
		, SuperRegion_Annual_FB_Quota = Geo_Quota.SuperRegion_Annual_FB_Quota
	
		, Geo = Geo_Quota.Geo
		, Geo_Qtrly_Quota = Geo_Quota.Geo_Qtrly_Quota
		, Geo_Half_Quota = Geo_Quota.Geo_Half_Quota
		, Geo_Annual_Quota = Geo_Quota.Geo_Annual_Quota

		, Geo_Qtrly_FB_Quota = Geo_Quota.Geo_Qtrly_FB_Quota
		, Geo_Half_FB_Quota = Geo_Quota.Geo_Half_FB_Quota
		, Geo_Annual_FB_Quota = Geo_Quota.Geo_Half_FB_Quota

		, SE_Quota = coalesce(SE_Quota.SE_Quota, Geo_Quota.Terr_Qtrly_Quota)
		, SE_Half_Quota = coalesce(SE_Quota.SE_Half_Quota , Geo_Quota.Terr_Half_Quota)
		, SE_Annual_Quota = coalesce(SE_Quota.SE_Annual_Quota, Geo_Quota.Terr_Annual_Quota)

		, FB_Quota = coalesce(SE_Quota.FB_Quota, Geo_Quota.Terr_Qtrly_FB_Quota)
		, SE_Half_FB_Quota = coalesce(SE_Quota.SE_Half_FB_Quota, Geo_Quota.Terr_Half_FB_Quota)
		, SE_Annual_FB_Quota = coalesce(SE_Quota.SE_Annual_FB_Quota, Geo_Quota.Terr_Annual_FB_Quota)
*/		
--		, SE_Quota.Resource_Group
	...
	from (
			...
			from PureDW_SFDC_Staging.dbo.Opportunity Oppt
				...
				left join [PureDW_SFDC_staging].[dbo].[User] Acct_Exec on Acct_Exec.Id = OpptSplit.SplitOwnerID				--left join #AE_Coverage AE_Coverage on AE_Coverage.EmployeeID = Acct_Exec.EmployeeNumber
...
-----	left join SalesOps_DM.dbo.Coverage_assignment_byTerritory Assign_SE on Assign_SE.Territory_ID = Acct_Exec.Territory_ID__c
    left join Anaplan_DM.dbo.SE_Territory_Alignment Assign_SE on Assign_SE.[Territory Code] = Acct_Exec.Territory_ID__c
...
[Final].District_Permission = 'WW_AMS_ENT_NEA_FRE'
--= 'WW_AMS_COM_NEA_GTH'
