...
(
	...
	/* Base products: FlashArray for FlashArray AE & SE, FlashBlade for FlashBlade AE & SE */
		...
		left join [SalesOps_DM].[dbo].[SE_Org_Quota] SE_Quota on (SE_Quota.EmployeeID = SE_oppt_owner.EmployeeNumber and SE_Quota.[Period] = left(cast(oppt.Close_Quarter__c as nvarchar(10)), 2))
...
(
	...
	select b2.*, SE_Half_Quota2.Quota [Half_Quota] from (
		...
		left join [SalesOps_DM].[dbo].[SE_Org_Quota] SE_Quota on (SE_Quota.EmployeeID = FA_SE.EmployeeNumber and SE_Quota.[Period] = left(cast(oppt.Close_Quarter__c as nvarchar(10)), 2))
		left join (Select EmployeeID, cast(Quota as decimal(18,2)) Quota from [SalesOps_DM].[dbo].[SE_Org_Quota] where Period = 'FY') 
		...
		and (Oppt.Transaction_Type__c is null or Oppt.Transaction_Type__c != 'ES2 Renewal')
		and Oppt.Theater__c like 'FlashBlade'
...
where a.Id like '0060z00001x0SO1AAM%'
--'0060z000020VmEPAA0'
-- 
